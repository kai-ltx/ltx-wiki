...
## [2026-09-07] ingest | Weekly update
Sources added: 1 file (Area1/LTX news: 0, Area2/Community: 0, Area3/Competitors: 1, Area4/Tutorials: 0). Research window: 2026-08-24 to 2026-09-07.
Pages created: none. Pages updated: [[competitor-landscape-overview]], [[wan-video]].

**Quiet-week note:** This was a genuinely light window. LTX's own release-notes and product-updates blog show no entries between 2026-08-11 (LTX-2.5 launch) and today -- confirmed by directly fetching ltx.io/release-notes and the product-updates blog. Extensive searches across all four research areas (LTX news, community reception, competitors, tutorials/integrations) returned either pre-window content already logged in prior entries, or material explicitly dated before 2026-08-24 (e.g., a Seedance 2.5 deep-dive dated 2026-07-15, an LTX Studio review last edited 2026-03-24, a Lightricks layoffs report dated 2026-05-27). None of that pre-window material was re-ingested.

The one genuinely new, in-window finding: the Artificial Analysis video leaderboard refreshed since 2026-08-24 -- Wan 3.0's API pricing is now published ($12.00/min, resolving the prior "Coming soon" placeholder and further corroborating that Wan 3.0 is closed, not open-weights), and two new entrants appeared (Minimax H3 Max post-trained by fal, debuting #3 T2V-with-audio at a third of base MiniMax H3's cost; and Agnes-Video-2.5 from Sapiens AI, debuting #19). LTX-2.5's own Elo is essentially flat (1,062/1,061 vs 1,063/1,063) -- its rank slipped from #19-20 to #22/#24 purely because of these new entrants, not a quality change. No new frontier releases from Runway, Veo, Kling, Sora, Wan, Seedance, or HunyuanVideo were found in-window beyond what prior entries already cover.

Key findings: (1) Wan 3.0 API pricing confirmed at $12.00/min via AA, still no open-weights badge; (2) Minimax H3 Max (fal post-trained) and Agnes-Video-2.5 are new AA leaderboard entrants; (3) LTX-2.5 Elo stable, rank cosmetic-only change; (4) LTX shipped nothing new since the 2026-08-11 LTX-2.5 launch as of this run's retrieval.

## [2026-09-07] lint | Weekly health check
Broken links: 0 introduced this week (verified all wikilinks in the 2 updated pages resolve to existing files; escaped-pipe table-cell links `[[page\|Display]]` parse correctly and are not broken). Index drift: 0 (all 228 wiki files present in index.md, zero dangling index entries -- no new pages were created this run so no index edit was needed). Format issues: 0 (both updated pages retain complete frontmatter with correct `updated: 2026-09-07` and the new raw source appended to their `sources:` list). Pre-existing artifacts (unchanged from prior entries, not re-verified this run): backslash-escaped table-cell wikilinks noted in the 2026-07-21 entry are a markdown-table syntax pattern, not broken links.
